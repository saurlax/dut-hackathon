...
- 参赛队伍最终确认与成员快照
- 作品提交、公开展示和审核
- 基于角色的管理后台，支持待审核筛选、详情查看、审核和管理员账户管理
- 桌面端与移动端响应式界面
...
- 组队大厅只展示仍在截止日期内且状态为招募中的队伍；手动停止招募会进入暂停状态，重新保存可恢复招募，最终确认后才进入完成状态。
...
- 最终确认要求全员已确认，保存成员快照，并禁止继续修改名单或转让队长。
- 作品默认私密；作品及其关联队伍均已授权公开且审核通过后，作品才进入展示页。
- 管理员权限始终在服务端校验，前端按钮可见性不作为安全边界。
- 只有当前管理员可以在后台新增管理员；尚未注册的邮箱会预创建账户，并在首次魔法链接登录后继续保留管理员角色。
